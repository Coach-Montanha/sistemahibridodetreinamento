...
import { Languages, Play, Pause, Loader2, Info } from "lucide-react";
...
export function CatalogTranslationManager() {
    ...
    const processMutation = useMutation({
        mutationFn: (args) => processCatalogTranslationBatch({ data: args }),
        onSuccess: (res) => {
    ...
    const handleStartProcessing = (jobId) => {
    ...
    return (<div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          {!job || isCompleted ? (<div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
              ...
              <div className="flex justify-center gap-2">
                {!isProcessing ? (<Button onClick={() => handleStartProcessing(job.id)}>
                    <Play className="mr-2 h-4 w-4"/> Retomar
                  </Button>) : (<Button variant="outline" onClick={handlePause}>

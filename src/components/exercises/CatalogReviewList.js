...
import { translateSingleExercise, saveCatalogTranslationDraft, approveCatalogTranslation } from "@/lib/exercises-import.functions";
export function CatalogReviewList() {
    ...
    const updateStatus = useMutation({
        mutationFn: async ({ id, status, approved }) => {
    ...
    if (error) {
        return (<div className="rounded-md border border-destructive/50 bg-destructive/5 p-8 text-center">
    ...
    return (<div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
        <div className="flex-1 max-w-sm">
          <Input placeholder="Buscar por nome..." value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} className="h-9"/>
        </div>
            ...
            {!items || items.length === 0 ? (<TableRow>
                ...
                return (<>
                          <div className="flex flex-col">
                            ...
                            {translation?.name_pt_br && (<span className="text-sm text-primary">{translation.name_pt_br}</span>)}
                ...
                return (<div className="flex flex-col gap-1">
                          ...
                          {translation?.equipment_pt_br && (<Badge variant="secondary" className="text-[10px]">{translation.equipment_pt_br}</Badge>)}
                        </div>);
            })()}
                  </TableCell>
                  <TableCell>
                ...
                return <TranslationStatusBadge translation={translation}/>;
                  ...
                  <TableCell>
                    {item.projected_exercise_id ? (<Badge variant="secondary" className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                        Concluída
                      </Badge>) : item.approved_for_projection ? (<Badge variant="secondary" className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                        Aprovada
                      </Badge>) : (<Badge variant="outline">Pendente</Badge>)}
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon" title="Traduzir via IA" onClick={async () => {
                try {
                    ...
                    toast.error(err.message);
                      ...
                      {!item.projected_exercise_id && (<>
                          <Button variant="ghost" size="icon" className="text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => updateStatus.mutate({ id: item.id, status: 'approved', approved: true })}>
                            <Check className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" className="text-destructive" onClick={() => updateStatus.mutate({ id: item.id, status: 'rejected', approved: false })}>
                            <X className="h-4 w-4"/>
                          </Button>
                        </>)}
                    </div>
      ...
      {totalPages > 1 && (<div className="flex items-center justify-between px-4 py-4 border-t">
...
function TranslationStatusBadge({ translation }) {
    if (!translation)
        return <Badge variant="outline" className="opacity-50">Nenhuma</Badge>;
...
function CatalogItemReview({ item, onUpdated }) {
    const queryClient = useQueryClient();
    ...
    return (<div className="space-y-6 py-4">
      <div className="flex justify-between items-center px-1">
        <div className="flex items-center gap-2">
          ...
          {translation?.translation_status === 'draft' && (<Badge variant="secondary" className="bg-amber-100 text-amber-700">Rascunho IA</Badge>)}
        </div>
        <Button variant="outline" size="sm" onClick={() => translateMutation.mutate()} disabled={translateMutation.isPending} className="gap-2">
          ...
          <div className="space-y-4 border-l pl-8">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={edited.name_pt_br} onChange={e => setEdited(prev => ({ ...prev, name_pt_br: e.target.value }))} placeholder="Ex: Supino Reto com Halteres"/>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-xs">Equipamento</Label>
                  <Input value={edited.equipment_pt_br} onChange={e => setEdited(prev => ({ ...prev, equipment_pt_br: e.target.value }))} placeholder="Ex: Halteres"/>
                </div>
                <div>
                  <Label className="text-xs">Categoria</Label>
                  <Input value={edited.category_pt_br} onChange={e => setEdited(prev => ({ ...prev, category_pt_br: e.target.value }))} placeholder="Ex: Força"/>
                </div>
              </div>

              <div>
                <Label className="text-xs">Instruções Traduzidas</Label>
                <Textarea value={edited.instructions_pt_br} onChange={e => setEdited(prev => ({ ...prev, instructions_pt_br: e.target.value }))} placeholder="Passo a passo em português..." rows={12} className="text-sm"/>
              </div>
            </div>

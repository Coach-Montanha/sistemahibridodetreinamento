...
export function ExerciseImportManager() {
    ...
    return (<div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        <StatCard title="Fonte vs Catálogo" value={`${stats?.total ?? 0} / ${stats?.expected ?? 0}`} icon={Database} loading={statsLoading} color={(stats?.total || 0) < (stats?.expected || 0) ? "text-amber-500" : "text-green-500"}/>

        <StatCard title="Aguardando Revisão" value={stats?.pending ?? 0} icon={AlertCircle} color="text-amber-500" loading={statsLoading}/>
        <StatCard title="Prontos p/ Projeção" value={stats?.approved ?? 0} icon={CheckCircle2} color="text-green-500" loading={statsLoading}/>
        <StatCard title="Já Projetados" value={stats?.projected ?? 0} icon={RefreshCw} loading={statsLoading}/>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={handleTranslation} disabled={isTranslating || (stats?.needTranslation ?? 0) === 0}>
                {isTranslating ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Languages className="mr-2 h-4 w-4"/>}
                Traduzir via IA
              </Button>
              <Button onClick={handleProjection} disabled={isProjecting || (stats?.approved ?? 0) === 0}>
      ...
      <CardContent className="pt-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            {loading ? (<Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>) : (<p className={cn("text-2xl font-bold", color)}>{value}</p>)}
          </div>

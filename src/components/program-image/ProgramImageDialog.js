...
import { ImageDown, FileDown, Loader2, HelpCircle, Download, Sparkles, LayoutGrid, } from "lucide-react";
...
import { carregarLayout, salvarLayout, PRESETS_LAYOUT } from "@/lib/program-image-layout";
import { METHODOLOGY_LABEL } from "@/lib/methodology";
import { prepararSessoesParaImagem } from "@/lib/session-image";
import { exportarSessoesEmMassa, exportarSessoesPDF, } from "@/lib/image-export";
import { UnifiedCanvasEditor } from "./UnifiedCanvasEditor";
function idsDasSessoes(programa) {
    return (programa.program_weeks ?? [])
        .slice()
        .sort((a, b) => (a.numero_semana ?? 0) - (b.numero_semana ?? 0))
        .flatMap((w) => (w.sessions ?? [])
        .slice()
        .sort((a, b) => (a.numero_dia ?? 0) - (b.numero_dia ?? 0))
        .map((s) => String(s.id)));
}
...
export function useNovidadesPendentes() {
    const [pendente, setPendente] = useState(false);
    useEffect(() => setPendente(novidadesPendentes()), []);
    return pendente;
}
const ANEL = "ring-2 ring-primary/50 ring-offset-2 ring-offset-background rounded-md transition-shadow duration-200";
function GuiaDeUso({ realcado }) {
    ...
    return (<Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("h-8 w-8 shrink-0 text-muted-foreground transition-colors duration-200 hover:text-foreground", realcado && ANEL)} aria-label="Como usar o layout de imagem">
      ...
      <PopoverContent align="end" className="w-[min(22rem,calc(100vw-2rem))] p-0">
        ...
        <ul className="space-y-4 px-4 py-4">
          {passos.map((p) => (<li key={p.titulo} className="flex gap-3">
...
export function ProgramImageDialog({ programa, onOpenChange, }) {
    const open = !!programa;
    ...
    const modalidadeLabel = modalidade
        ? (METHODOLOGY_LABEL[modalidade] ?? modalidade)
        : null;
    useEffect(() => {
        if (open)
            setNovidades(novidadesPendentes());
    }, [open]);
    useEffect(() => {
        if (!destaque)
            return;
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelado)
                    toast.error(e?.message ?? "Não foi possível carregar as sessões");
            }
        })();
        return () => {
            cancelado = true;
    ...
    async function exportar(formato) {
        if (!layout || !sessoes || sessoes.length === 0)
            return;
      ...
      <DialogContent className="max-h-[92vh] max-w-5xl overflow-hidden p-0">
        <DialogHeader className="space-y-2 border-b border-border/60 px-4 py-4 sm:px-6">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
            <div className="min-w-0 space-y-1.5">
              <DialogTitle className="flex flex-wrap items-center gap-2 text-base leading-tight">
                Layout de imagem
                {programa?.titulo && (<Badge variant="secondary" className="max-w-[16rem] truncate font-normal">
                    {programa.titulo}
                  </Badge>)}
              </DialogTitle>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {modalidadeLabel && (<span className="text-[11px] leading-snug text-muted-foreground">
        ...
        <ScrollArea className="max-h-[72vh]">
          <div className="flex flex-col gap-6 px-6 py-5">
            {novidades && (<div className="rounded-xl border border-primary/30 bg-primary/[0.04] p-4 duration-200 animate-in fade-in">
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/40 p-1">
                    {Object.entries(PRESETS_LAYOUT).map(([id, p]) => (<Button key={id} variant="ghost" size="sm" className={cn("h-7 px-2 text-[10px] uppercase font-bold", layout?.largura === p.layout.largura && "bg-background shadow-sm")} onClick={() => {
                if (!layout)
                    return;
                ...
                salvarLayout(programa.id, next);
                  ...
                  <Button variant="outline" size="sm" className="h-7 gap-1 text-[10px] uppercase font-bold" onClick={() => {
            if (!layout)
                return;
            ...
            salvarLayout(programa.id, next);
              ...
              {layout && sessoes && sessoes.length > 0 ? (<UnifiedCanvasEditor layout={layout} blocos={sessoes[0].input.principal} metodologiaLabel={sessoes[0].input.metodologiaLabel} coachLabel={sessoes[0].input.coachLabel} onChange={(newLayout) => {
                setLayout(newLayout);

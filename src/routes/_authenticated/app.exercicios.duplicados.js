...
import { GitMerge, Trash2, ChevronLeft, Loader2, Globe2 } from "lucide-react";
...
function DuplicadosPage() {
    ...
    return (<div className="mx-auto max-w-4xl p-6">
      ...
      {duplicates.length === 0 ? (<Card className="flex flex-col items-center justify-center p-12 text-center">
          ...
          <Button className="mt-6" onClick={() => navigate({ to: "/app/exercicios" })}>
            Voltar ao Banco
          </Button>
        </Card>) : (<div className="space-y-6">
          {duplicates.map((group) => (<Card key={group.name} className="overflow-hidden border-border/60">
              ...
              <div className="divide-y divide-border/40">
                {group.items.map((ex) => {
                    ...
                    return (<div key={ex.id} className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between hover:bg-accent/5 transition-colors">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{ex.nome_pt}</span>
                          {isGlobal && (<Badge variant="outline" className="gap-1 text-[10px] uppercase">
                              <Globe2 className="h-3 w-3"/> Global
                            </Badge>)}
                        </div>
                        <div className="mt-1 flex flex-wrap gap-1">
                          {ex.equipamento?.map((eq) => (<Badge key={eq} variant="secondary" className="text-[10px]">{eq}</Badge>))}
                          {ex.metodologias?.map((m) => (<Badge key={m} variant="outline" className="text-[10px]">{METHODOLOGY_LABEL[m]}</Badge>))}
                        </div>
                      </div>

                      <div className="flex shrink-0 gap-2">
                        {!isGlobal && (<Button variant="outline" size="sm" className="h-8 gap-1.5" onClick={async () => {
                                const others = group.items.filter((i) => i.id !== ex.id).map((i) => i.id);
                        ...
                        {!isGlobal && (<Button variant="ghost" size="sm" className="h-8 text-destructive hover:bg-destructive/10" onClick={async () => {

...
export function montarHibridoPrompt(args: {
  ...
  return [
    ...
    filosofia,
    "",
    `SUA TAREFA É EVOLUIR A PROGRAMAÇÃO (CONTINUAR GERANDO):`,
    ...
    `Você deve analisar o HISTÓRICO COMPLETO abaixo para projetar a sobrecarga progressiva e variação técnica.`,
    `VOCÊ TEM LIBERDADE TOTAL PARA ALTERAR:`,
    `- Escolher novos exercícios (do pool de candidatos) para variar o estímulo.`,
    `- Aumentar/diminuir o número de exercícios dentro do limite do bloco.`,
    `- Progredir cargas e volume baseando-se no que já foi feito.`,
    `- Regredir ou progredir a dificuldade técnica dos movimentos se necessário para a evolução.`,
    "",
    `REGRAS DE SELEÇÃO:`,

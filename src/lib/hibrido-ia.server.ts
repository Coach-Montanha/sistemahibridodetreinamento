...
1. Mobilidade: selecionar automaticamente um movimento do grupo Mobilidade, com duração exata de 2'.
...
export function montarHibridoPrompt(args: {
  ...
  return [
    ...
    `Gere ${payload.numeroSessoes} sessão(ões) em sequência usando exatamente este mesmo molde de blocos em todas elas. Ao longo da sequência, varie a escolha de exercícios entre as sessões sempre que o pool de candidatos permitir (evite repetir a mesma combinação exata em todas as sessões), mas nunca sacrifique a variedade de padrão de movimento dentro de um único bloco por causa disso — dentro de cada bloco, prefira exercícios que se complementem (evite dois exercícios muito parecidos no mesmo bloco quando o pool oferece alternativas diferentes).`,
    "",

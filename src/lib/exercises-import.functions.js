...
export const saveCatalogTranslationDraft = createServerFn({ method: "POST" })
    ...
    await supabaseAdmin
        .from("exercise_catalog")
        .update({ review_status: 'approved', approved_for_projection: true })
...
export const projectApprovedExercises = createServerFn({ method: "POST" })
    .handler(async () => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const fetchAndProject = async (offset = 0) => {
        ...
        for (const item of approved) {
            const translation = item.exercise_catalog_translations?.find((t) => t.locale === 'pt-BR' && t.translation_status === 'approved')

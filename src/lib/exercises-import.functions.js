...
export const importExercises = createServerFn({ method: "POST" })
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Falha ao baixar dataset: ${response.statusText}`);
...
async function translateCatalogExerciseInternal(catalogId, locale = "pt-BR") {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const projectApprovedExercises = createServerFn({ method: "POST" })
    ...
    for (const item of approved) {
        const translation = Array.isArray(item.exercise_catalog_translations)
            ? item.exercise_catalog_translations.find((t) => t.translation_status === 'approved')
            : item.exercise_catalog_translations?.translation_status === 'approved'
        ...
        const muscleGroups = [
            ...
            ...(Array.isArray(translation.secondary_muscles_pt_br)
                ? translation.secondary_muscles_pt_br
...
export const saveCatalogTranslationDraft = createServerFn({ method: "POST" })
    ...
    if (transError)
        throw new Error(`Falha ao salvar tradução: ${transError.message}`);
    ...
    const { error: catalogError } = await supabaseAdmin
        ...
        .eq("id", catalogId);
    if (catalogError)
        throw new Error(`Falha ao atualizar status do catálogo: ${catalogError.message}`);
    return { success: true };
});
export const approveCatalogTranslation = createServerFn({ method: "POST" })
    ...
    const { error } = await supabaseAdmin
        ...
        .eq("id", catalogId);
    if (error)
        throw new Error(`Erro ao aprovar exercício: ${error.message}`);
    return { success: true };
});

 ...
 */
export function resolveBaseFormat(formatId) {
    if (formatId.startsWith("builtin:"))
        return formatId.replace("builtin:", "");
    if (formatId.startsWith("custom:"))
        return formatId; // Will need lookup if custom formats have different base
    return formatId;
}
...
export function normalizarEquipamento(valor) {
    const k = chaveEquip(valor);
    const achado = EQUIPAMENTOS_CANONICOS.find((c) => chaveEquip(c) === k);
    if (achado)
        return achado;
    if (k === "ginastico" || k === "ginasticos")
        return "Ginásticos";
    if (["cable", "machine", "plate"].includes(k))
        return "Alternativos Musculação";
    return null;
}
/**
...
supabase, template) {
    ...
    for (const bloco of template) {
        ...
        if (error)
            throw new Error(`Falha ao buscar exercícios para o bloco "${bloco.chave}": ${error.message}`);
        const dataArr = data ?? [];
        if (bloco.selecaoExercicios === "ia" && dataArr.length === 0) {
            const equipDesc = equipamentosNormalizados.length > 0
        ...
        out[bloco.chave] = dataArr.map((e) => ({ id: e.id, nome: e.nome_pt }));
    }
    return out;
}
...
export function montarHibridoPrompt(args) {
    ...
    const blocosDescricao = payload.sessaoTemplate.map((b) => {
        ...
        return {
            ...
            candidatos_permitidos: b.selecaoExercicios === "ia" ? pool.map((c) => ({ id: c.id, nome: c.nome })) : undefined,
        };
    });
    return [
 ...
 */
export function normalizarPrescricaoHibrido(bruto, template, candidatos) {
    let json;
    try {
        const limpo = bruto
            .trim()
    ...
    const sessoes = sessoesRaw.map((s) => {
        ...
        const blocos = template.map((bt) => {
            ...
            const idsRecebidos = Array.isArray(recebido?.exercicios_ids)
                ? recebido.exercicios_ids.filter((x) => typeof x === "string")
                : [];

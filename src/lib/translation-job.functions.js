...
export const startCatalogTranslationJob = createServerFn({ method: "POST" })
    ...
    const { data: coachId } = await supabaseAdmin.rpc("auth_coach_id_for_user", { _user_id: context.userId });
    if (!coachId)
        throw new Error("Coach ID não resolvido.");
    ...
    if (!pendingExercises || pendingExercises.length === 0) {
        return { success: false, message: "Nenhum exercício pendente no catálogo." };
    }

...
import { SYSTEM_PROMPT, calcularDataFim, montarUserPrompt, normalizarPrescricao, } from "@/lib/prescricao-ia.server";
import { KB_SPORT_SYSTEM_PROMPT, escolherEscola, montarKbSportPrompt, } from "@/lib/kb-sport-ia.server";
import { WL_SYSTEM_PROMPT, escolherEscolaWl, montarWlPrompt, } from "@/lib/weightlifting-ia.server";
import { TF_SYSTEM_PROMPT, escolherEscolaFuncional, montarFuncionalPrompt, } from "@/lib/funcional-ia.server";
import { CO_SYSTEM_PROMPT, escolherEscolaCorrida, montarCorridaPrompt, } from "@/lib/corrida-ia.server";
import { buscarCandidatosDoMolde, montarHibridoPrompt, normalizarPrescricaoHibrido, } from "@/lib/hibrido-ia.server";
...
export const prescribeTrainingWithAi = createServerFn({ method: "POST" })
    ...
    const { data: programa, error } = await supabase
        .from("programs")
        .select("id, titulo, metodologia, descricao, data_inicio, duracao_semanas, program_weeks(id, numero_semana)")
        .eq("id", data.programId)
        .maybeSingle();
    if (error)
        throw new Error(error.message);
    if (!programa)
        throw new Error("Programa não encontrado ou sem permissão de acesso");
    ...
    if (metodologiaEfetiva !== "musculacao" &&
        ...
        throw new Error("Prescrever com IA está disponível apenas para Musculação, Híbrido, Kettlebell Fitness, Kettlebell Sport, Levantamento de Peso, Treinamento Funcional e Corrida");
    }
    ...
    const linha = kb
        ? data.escolaOverride || (kb.escolaMetodologica === "auto"
            ? escolherEscola(kb.nivelAtleta, kb.disciplina)
            : kb.escolaMetodologica)
        : (metodologiaEfetiva === "kettlebell_sport" ? data.escolaOverride || "gomonov" : null);
    const wl = data.wl ?? null;
    const linhaWl = wl
        ? data.escolaOverride || (wl.escolaMetodologica === "auto"
            ? escolherEscolaWl({
        ...
        : (metodologiaEfetiva === "levantamento_peso" ? data.escolaOverride || "takano" : null);
    const tf = data.tf ?? null;
    const linhaTf = tf
        ? data.escolaOverride || (tf.escolaMetodologica === "auto"
        ...
        : (metodologiaEfetiva === "treinamento_funcional" ? data.escolaOverride || "exos" : null);
    const co = data.co ?? null;
    const linhaCo = co
        ? data.escolaOverride || (co.escolaMetodologica === "auto"
        ...
        : (metodologiaEfetiva === "corrida" ? data.escolaOverride || "daniels" : null);
        ...
        : isWl
            ...
            : isTf
                ...
                : isCo
                    ? CO_SYSTEM_PROMPT
                    : SYSTEM_PROMPT;
    const userPrompt = isCo && co && linhaCo
        ? montarCorridaPrompt({
            co: co,
            linha: linhaCo,
    ...
    try {
        if (isHibrido) {
            const templateFinal = data.hibrido.sessaoTemplate?.length > 0
                ...
                : continuation.lastSessionStructure
                    ? continuation.lastSessionStructure.map((b) => ({
                        chave: b.chave,
                        formato: b.formato,
                        titulo: b.titulo,
                        selecaoExercicios: "ia",
            ...
            const result = normalizarPrescricaoHibrido(conteudo, templateFinal, await buscarCandidatosDoMolde(supabase, templateFinal));
            return { ...result, generation_source: "ai" };
        }
        ...
        if (parseErr.message.includes("AI_") || parseErr.message.includes("POOL_VAZIO"))
            throw parseErr;
